...
internal static class AuthenticationContextKeys {
	...
	/// <c>SignalRConnection.Items</c> at <c>InvocationContextHubFilter.OnConnectedAsync</c>
	/// and seeded onto per-invocation <c>IInvocationContext.Items</c> at
	/// <c>SignalRInvocationContext</c> construction so consumers of the per-invocation bag
	/// (e.g. <c>UserStateAccessor</c>) read identically across HTTP and SignalR.
	/// </summary>
